...
indice %>% 
  transmute(
    orgao,
    data = as.Date(str_glue("{ano}-{mes}-01")),
    status,
...
indice %>% 
  transmute(
    ...
    ord = case_when(
      necessidade_captcha == "Limite de esforço" ~ 0,
      necessidade_captcha == "Órgão não prestou contas" ~ 0.1,
      necessidade_captcha == "Requer captcha" ~ 0.5,
      necessidade_captcha == "Não requer captha" ~ 1
    )
  ) %>% 
  ggplot(aes(
    x = data,
