...
indice_long <- indice %>% 
  transmute(
    ...
    tem_cargo,
    tem_matricula,
    across(c(receita_base, despesas, outras_receitas), ~ case_when(
      . == "DETALHADO" ~ 1,
      . == "SUMARIZADA" ~ 0.5,
      TRUE ~ 0
    )),
  ...
  complete(orgao, ano, mes, criterio, fill = list(pontuacao = 0)) %>% 
  mutate(
    criterio = criterio %>% 
      str_replace_all("_", " ") %>% 
      str_to_sentence() %>% 
    ...
    dimensao = if_else(criterio %in% c(
        ...
        "Receita base", "Outras receitas", "Despesas"), "Completude",
        "Facilidade"
    )
  ) 

# indice_long %>% 
#   group_by(orgao, criterio) %>% 
#   summarise(pontuacao = mean(pontuacao), .groups = "drop") %>% 
#   arrange(dimensao, orgao, criterio) %>% 
#   googlesheets4::write_sheet(
#     "1Ke2MhFRh5yWwHRCZBr-WHvuh4y9dzs0JkBI27sFR2Fk",
#     sheet = "Categorias")
```
